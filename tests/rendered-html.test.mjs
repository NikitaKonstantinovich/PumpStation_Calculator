...
test("provides a persisted DN calculator with SP velocity defaults", async () => {
  ...
  assert.match(projectConfig, /export type ValveConnection = "threaded" \| "flanged"/);
  assert.match(projectConfig, /export type ValvePn = 10 \| 16 \| 25/);
  assert.match(projectConfig, /"station-dn": \{ kind: "dn"/);
  assert.match(projectConfig, /collectorMaterial:rawDn\.collectorMaterial==="st20"/);
  assert.match(projectConfig, /secondarySuctionCollectorDn:number\|null/);
  assert.match(projectConfig, /secondaryConnectionType:ValveConnection\|null/);
  assert.match(styles, /\.dn-calculator__grid/);
  assert.match(styles, /\.dn-calculator__controls/);
  assert.match(styles, /\.dn-circuit__color--red/);
