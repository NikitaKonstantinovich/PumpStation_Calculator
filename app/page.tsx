...
const STANDARD_DN=[25,32,40,50,65,80,100,125,150,200,250,300,350,400,450,500,600,700,800,900,1000,1200] as const;
type DnField="suctionCollectorDn"|"dischargeCollectorDn"|"suctionValveDn"|"dischargeValveDn";
const dnVelocityLimit=(dn:number)=>dn<=250?2:3;
const flowVelocity=(flowRate:number,dn:number)=>flowRate/3600/(Math.PI*(dn/1000)**2/4);
const recommendedDn=(flowRate:number)=>STANDARD_DN.find(dn=>flowVelocity(flowRate,dn)<=dnVelocityLimit(dn))??STANDARD_DN.at(-1)!;
const defaultCollectorMaterial=(settings:SettingsEntity):CollectorMaterial=>settings.stationType==="fire"?"st20":"aisi304";

function DnCalculator({entity,input,settings,onChange}:{entity:DnEntity;input:InputEntity;settings:SettingsEntity;onChange?:(patch:Partial<DnEntity>)=>void}){
  if(!isCalculated(input))return <div className="dn-calculator__empty"><span aria-hidden="true">Ø</span><b>Сначала задайте рабочую точку</b><p>Укажите расход станции и количество рабочих насосов в инструменте «Подбор насосов».</p></div>;
  ...
  const values:Record<DnField,number>={
    suctionCollectorDn:entity.suctionCollectorDn??collectorDn,
    dischargeCollectorDn:entity.dischargeCollectorDn??collectorDn,
    suctionValveDn:entity.suctionValveDn??pumpDn,
    dischargeValveDn:entity.dischargeValveDn??pumpDn,
  };
  const rows:Array<{field:DnField;title:string;subtitle:string;flow:number}>=[
    {field:"suctionCollectorDn",title:"Всасывающий коллектор",subtitle:"Расход всей станции",flow:stationFlow},
    {field:"dischargeCollectorDn",title:"Напорный коллектор",subtitle:"Расход всей станции",flow:stationFlow},
    {field:"suctionValveDn",title:"Арматура на всасе насоса",subtitle:"Расход одного рабочего насоса",flow:pumpFlow},
    {field:"dischargeValveDn",title:"Арматура на напоре насоса",subtitle:"Расход одного рабочего насоса",flow:pumpFlow},
  ];
  const reset=()=>onChange?.({suctionCollectorDn:collectorDn,dischargeCollectorDn:collectorDn,suctionValveDn:pumpDn,dischargeValveDn:pumpDn,collectorMaterial:defaultCollectorMaterial(settings)});
  return <div className="dn-calculator">
    <header className="dn-calculator__summary"><div><small>РАСЧЁТНЫЕ РАСХОДЫ</small><b>Станция: {stationFlow.toLocaleString("ru-RU")} м³/ч</b><span>Один насос: {pumpFlow.toLocaleString("ru-RU",{maximumFractionDigits:2})} м³/ч · рабочих насосов: {input.workingPumpCount}</span></div><button className="button" onClick={reset}>Заполнить по умолчанию</button></header>
    ...
    <div className="dn-calculator__grid">{rows.map(row=>{const dn=values[row.field],velocity=flowVelocity(row.flow,dn),limit=dnVelocityLimit(dn),valid=velocity<=limit+1e-9;return <section className="dn-calculator__card" key={row.field}><div className="dn-calculator__card-title"><span><b>{row.title}</b><small>{row.subtitle} · {row.flow.toLocaleString("ru-RU",{maximumFractionDigits:2})} м³/ч</small></span><i className={valid?"dn-calculator__status dn-calculator__status--ok":"dn-calculator__status dn-calculator__status--warning"}>{valid?"Норма":"Выше нормы"}</i></div><div className="dn-calculator__values"><label><span>Условный проход</span><select value={dn} onChange={event=>onChange?.({[row.field]:Number(event.target.value)})} aria-label={`Ду: ${row.title}`}>{STANDARD_DN.map(value=><option key={value} value={value}>Ду {value}</option>)}</select></label><span className="dn-calculator__speed"><small>Скорость потока</small><b>{velocity.toLocaleString("ru-RU",{minimumFractionDigits:2,maximumFractionDigits:2})} <i>м/с</i></b><em>Допустимо до {limit} м/с</em></span></div></section>;})}</div>
    <footer className="dn-calculator__note"><span>i</span><p><b>Автоподбор по СП 31.13330</b>Для Ду до 250 скорость ограничена 2 м/с, для Ду более 250 — 3 м/с. При ручном выборе Ду скорость пересчитывается сразу.</p></footer>
  </div>;
}

function PanelContent({ id, entity, input, settings, draftPoint, catalogue, onDraftPointChange, onInputChange, onSettingsChange, onDnChange, onAddToSpec, onCabinetConfigured, locked }: { id: PanelKind; entity: ProjectEntity; input: InputEntity; settings: SettingsEntity; draftPoint:{flowRate:string;head:string}; catalogue:Pump[]; onDraftPointChange:(point:{flowRate:string;head:string})=>void; onInputChange?: (patch: Partial<InputEntity>) => void; onSettingsChange?: (patch: Partial<SettingsEntity>) => void; onDnChange?:(patch:Partial<DnEntity>)=>void; onAddToSpec?:()=>void; onCabinetConfigured?:(cabinet:SmartCabinet)=>void; locked?:boolean }) {
  ...
  if (id === "dn") return <DnCalculator entity={entity as DnEntity} input={input} settings={settings} onChange={onDnChange}/>;
